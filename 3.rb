# Function to turn the wires into instructions
def parse_wire(wire)
  inputs = []
  wire.split(",").each do |input|
    inputs << [input[0],input[1..-1].to_i]
    # puts input[0]
    # puts input[1..-1].to_i
  end
  return inputs
...
def add_wire_to_grid(name,instructions)
  # Create a new array in the grid, all wires start at [0,0]
  @grid[name] = []
  @grid[name] << [0,0]
  @lengths[name] = Hash.new

  wire_length = 0

  instructions.each do |ins|

    lx = @grid[name].last[0] # last position of wire, x
    ly = @grid[name].last[1] # last position of wire, y

    direction = ins[0]
    length    = ins[1]
    # puts "D:#{direction} - #{length}"
    ...
    when "R"
      until i > length do
        ...
        # Add the length to our Hash
        wire_length += 1
        @lengths[name][@grid[name].last] = wire_length
      end
    when "L"
      until i > length do
        @grid[name] << [lx - i,ly]
...
def find_shortest_path(intersections)
  distances = Hash.new
  intersections.each do |x,y|
    # Only use positive values
    x = 0 - x if x < 0
    y = 0 - y if y < 0
    distances["#{x},#{y}"] = x + y unless x == 0 && y == 0
  end
  distances.sort_by {|k, v| v}
end

def find_shortest_wirelengths(intersections)
  distances = Hash.new
  intersections.each do |x,y|
    distances["#{x},#{y}"] = @lengths["Wire 1"][[x,y]] + @lengths["Wire 2"][[x,y]] unless x == 0 && y == 0
  end
  distances.sort_by {|k, v| v}
end
...
# Grid will hold all the wire values
@grid = Hash.new
# Lengths will hold the wire lengths
@lengths = Hash.new

wire1_instructions = parse_wire(wire1)
wire2_instructions = parse_wire(wire2)

add_wire_to_grid("Wire 1",wire1_instructions)
add_wire_to_grid("Wire 2",wire2_instructions)

# Check the intersection points
intersections = @grid["Wire 1"] & @grid["Wire 2"]

puts "Showing Shortest Paths"
puts find_shortest_path(intersections)

puts "Showing Shortest Wire lengths"
puts find_shortest_wirelengths(intersections)
